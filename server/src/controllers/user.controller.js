...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (role === UserRolesEnum.COMPANYMEMBER) {
    // Find existing company where this email is allowed
    ...
    if (!existingCompany) {
      throw new ApiError(
        403,
        "Email is not allowed to register under any company."
      );
    }
  ...
  const user = await User.create({
    ...
    role: role || UserRolesEnum.NORMALUSER,
    companyId,
  });
